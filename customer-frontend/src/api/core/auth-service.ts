...
export const AUTH_ENDPOINTS = {
  ...
  testVerifyOtp: "/api/auth/phone/test-verify",
...
/**
 * Dev/staging-only bypass — one fixed code accepted for any phone number,
 * no Firebase round trip. The backend 404s this route entirely whenever
 * APP_ENV=production, so calling it against a live deploy is a no-op that
 * falls straight through to the caller's normal error handling.
 */
export async function verifyPhoneOtpTestMode(
  phone: string,
  code: string,
  explicitRole?: AccountRole,
): Promise<AuthSession> {
