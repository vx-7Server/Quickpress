...
/** Search + multi-select filters + sort for the delivery history list. */
export function HistoryToolbar({
  ...
  onToggleFilter: (id: HistoryFilterId) => void;
  sort: HistorySortId;
  onSort: (next: HistorySortId) => void;
  resultCount: number;
}) {
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 rounded-2xl border border-border bg-card px-3 py-1 focus-within:border-primary/70">
        ...
        <input
          ...
          aria-label="Search history by order ID, customer or partner"
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
        <p className="truncate text-[0.7rem] font-semibold text-muted-foreground">
          {resultCount} {resultCount === 1 ? "delivery" : "deliveries"}
        </p>
        <label className="flex shrink-0 items-center gap-1.5 rounded-2xl border border-border bg-card px-2.5 py-2">
          ...
          <select
            value={sort}
            onChange={(event) => onSort(event.target.value as HistorySortId)}
            className="bg-transparent text-[0.7rem] font-black tracking-tight text-foreground outline-none"
          >
